...
class Generator(Runner):
    ...
    def __init__(self, model, nb_classes, cuda=False, cond=False):
        ...
        self.nb_classes = nb_classes
        self.cond = cond
    def __call__(self, n_seqs, seq_len, prompt_files=None):
        torch.backends.cudnn.enabled = True

        self.reset_hidden_states()

        bottom_frame_size = self.model.frame_level_rnns[0].n_frame_samples
        # Sequence size [num_samples, seq_length] initially filled with model_q_levels // 2
        sequences = torch.LongTensor(n_seqs*self.nb_classes, self.model.lookback + seq_len) \
                         .fill_(utils.q_zero(self.model.q_levels))

        seq_start = self.model.lookback

        if prompt_files:
            n_seqs = len(prompt_files)
            sequences = torch.LongTensor(n_seqs, self.model.lookback + seq_len) \
                .fill_(utils.q_zero(self.model.q_levels))
            prompt_length = prompt_length_in_seconds * sample_rate
            seq_start = self.model.lookback + prompt_length
            labels = []
            for i,file in enumerate(prompt_files):
                seq_label = utils.load_audio_prompt(file, self.model.q_levels,
                                        self.model.lookback,
                                        prompt_length
                                        )
                label = seq_label[-1]
                ...
                sequences[i, :self.model.lookback + prompt_length] = seq
                labels.append(label)
            self.cond = labels
        ...
        for i in range(seq_start, self.model.lookback + seq_len): # stepping through sequence
            for (tier_index, rnn) in \
                    reversed(list(enumerate(self.model.frame_level_rnns))): # Stepping through frame level RNNs
