...
class GeneratorPlugin(Plugin):
    ...
    def epoch(self, epoch_index, prompt_files):
        if prompt_files:
            num_seqs = len(prompt_files)
        else:
            num_seqs = self.n_samples*self.nb_classes
        
        samples = self.generate(self.n_samples, self.sample_length, prompt_files=prompt_files) \
                      .cpu().float().numpy()
        for i in range(num_seqs):
            write_wav(
                os.path.join(
                    self.samples_path, self.pattern.format('a', i + 1)
                ),
